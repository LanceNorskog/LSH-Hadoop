...
public abstract class ItemDim2PrefsMapper extends
    Mapper<LongWritable,Text, ItemDimWritable,PrefUserSpotWritable> {
  ...
  ItemDim2PrefsMapper(boolean itemKey) {
  ...
  public void map(LongWritable key,
    ...
    String[] tokens = ItemDim2PrefsMapper.DELIMITER.split(value.toString());
    ...
    for(int dim = 0; dim < DIM; dim++) {
        ItemDimWritable keyout = new ItemDimWritable(userID, dim);
		float prefValue = tokens.length > 2 ? Float.parseFloat(tokens[2]) : 1.0f;
		float spotValue = dimensionRandom[dim].nextFloat();
    	keyout.set(itemID, dim);
    	PrefUserSpotWritable valueout = new PrefUserSpotWritable(userID, prefValue, spotValue);
