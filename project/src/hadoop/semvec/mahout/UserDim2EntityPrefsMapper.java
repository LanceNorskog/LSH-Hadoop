...
public abstract class UserDim2EntityPrefsMapper extends
    Mapper<LongWritable,Text, UserDimWritable,EntityPrefWritable> {
  ...
  UserDim2EntityPrefsMapper(boolean itemKey) {
  ...
  public void map(LongWritable key,
    ...
    String[] tokens = UserDim2EntityPrefsMapper.DELIMITER.split(value.toString());
    ...
    UserDimWritable keyout = new UserDimWritable(userID, 0);
    for(int dim = 0; dim < DIM; dim++) {
    	keyout.set(userID, dim);
    	if (booleanData) {
    		context.write(keyout, new EntityPrefWritable(itemID, 1));
    	} else {
    		float prefValue = tokens.length > 2 ? Float.parseFloat(tokens[2]) : 1.0f;
    		context.write(keyout, new EntityPrefWritable(itemID, prefValue));
    	}
    }
  }
