...
public final class FastIDSetTest extends TasteTestCase {
  ...
  public void testIntersectionSize() {
    FastIDSet set = buildTestFastSet();
    FastIDSet other = new FastIDSet();
    other.add(2L);
    other.add(100L);
    assertTrue(set.intersectionSize(other) == 1);
    assertTrue(other.intersectionSize(set) == 1);

  }

  @Test
  public void testRetainAll() {
    FastIDSet testSet = buildTestFastSet();
    FastIDSet set = buildTestFastSet();
    FastIDSet otherSet = new FastIDSet();
    set.add(100L);
    otherSet.add(2L);
    otherSet.add(100L);
    assertTrue(set.intersectionSize(otherSet) == 2);
    assertTrue(otherSet.intersectionSize(set) == 2);
    set.retainAll(otherSet);
    assertTrue(set.intersectionSize(testSet) == 1);
    assertTrue(set.intersectionSize(otherSet) == 2);
    assertTrue(otherSet.intersectionSize(set) == 2);
    LongPrimitiveIterator it = set.iterator();
    int count = 0;
    ...
    assertTrue(otherSet.contains(2L));
    assertTrue(otherSet.contains(100L));
  }
