...
public class SVProjectorRP {
  ...
  private static void svdOrig(List<NamedVector> itemsOrig) {
    Matrix origMatrix = getMatrix(itemsOrig);
   ...
   */
  private static List<NamedVector> reProject(String pathX, List<NamedVector> itemsOrig, List<NamedVector> itemsRP, MetadataModel itemNames)
      throws TasteException, IOException, FileNotFoundException {
    report("Reprojection:");
    Matrix q = getQ(itemsRP);
    Matrix itemsOrigQ = null;
    itemsOrigQ = getMatrix(itemsOrig);
    
    // SAMPLES x TARGET_DIMS
    Matrix qt = q.transpose();
    Matrix origRPQ = qt.times(itemsOrigQ);
    SingularValueDecomposition svd = new SingularValueDecomposition(origRPQ);
    double[] singulars = svd.getSingularValues();
    report("Singulars:\n");
    System.out.println("\t" + Arrays.toString(singulars));
    List<NamedVector> repro = new ArrayList<NamedVector>();
    for(NamedVector v: itemsRP) {
      repro.add(new NamedVector(q.times(v), v.getName()));
    }
  ...
  private static Matrix getMatrix(List<NamedVector> itemsOrig) {
    Matrix items = new DenseMatrix(itemsOrig.size(), SOURCE_DIMENSIONS);
    for(int i = 0; i < itemsOrig.size(); i++) {
      items.assignRow(i, itemsOrig.get(i));
    }
    return items;
  }
  ...
  private static void printMatrix(String matrixPath,
    ...
    for(int r = 0; r < matrix.numRows(); r++) {
      String label = itemsOrig.get(r).getName();
      ps.print(r + "," + label.trim().replaceAll(",", "\",\"") + ",");
      for(int c = 0; c < matrix.rowSize(); c++) {
