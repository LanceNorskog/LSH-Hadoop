 ...
 */
abstract class AbstractDifferenceRecommenderEvaluatorSimple implements RecommenderEvaluator {

  private static final Logger log = LoggerFactory.getLogger(AbstractDifferenceRecommenderEvaluatorSimple.class);

  private final Random random;
  private float maxPreference;
  private float minPreference;

  AbstractDifferenceRecommenderEvaluatorSimple() {
  ...
  public final double evaluate(RecommenderBuilder recommenderBuilder,
    ...
    return evaluateDual(recommenderBuilder, dataModelBuilder, dataModel,
        null, trainingPercentage, evaluationPercentage);
  }

  public double evaluateDual(RecommenderBuilder recommenderBuilder,
  ...
  //	public final double evaluate2(Recommender recommender, 
  //			DataModel masterDataModel) throws TasteException {
  //		int numUsers = masterDataModel.getNumUsers();
  //		FastByIDMap<PreferenceArray> userPrefs = new FastByIDMap<PreferenceArray>(
  //				 numUsers);
  //		double trainingResult = getEvaluation(userPrefs, recommender);
  //		log.info("Evaluation result: {}", trainingResult);
